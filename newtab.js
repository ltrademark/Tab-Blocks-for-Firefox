...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function createLinkElement(link, collectionId) {
    ...
    linkFavicon.alt = '';
    linkFavicon.onerror = () => { linkFavicon.style.display = 'none'; };
  ...
  function createCollectionElement(collection, filteredLinks) {
    ...
    if (filteredLinks && filteredLinks.length > 0) {
        filteredLinks.forEach(link => { linksGrid.appendChild(createLinkElement(link, collection.id)); });
    } else {
  ...
  function renderCollections() {
    ...
    const filteredCollections = collections.filter(collection => {
        ...
        const linkMatch = collection.links.some(link =>
    ...
    filteredCollections.forEach(collection => {
        ...
        const filteredLinks = !lowerSearchTerm ? collection.links : collection.links.filter(link =>
             (link.title || '').toLowerCase().includes(lowerSearchTerm) ||
             (link.url || '').toLowerCase().includes(lowerSearchTerm)
        );
  ...
  function handleLinkDragOver(event) { // Handles BOTH tab links and link blocks over a collection
    event.preventDefault();
  ...
  function handleLinkDragLeave(event) { // Handles BOTH tab links and link blocks leaving a collection
    const collectionDiv = event.target.closest('.collection');
  ...
  function handleDrop(event) { // Unified Drop Handler for Collections
    event.preventDefault();
    const collectionDiv = event.target.closest('.collection');
    if (!collectionDiv) { cleanupAfterDrop(); return; }

    const targetCollectionId = collectionDiv.dataset.collectionId;
    const targetCollection = collections.find(c => c.id == targetCollectionId);
    if (!targetCollection) { console.error("Drop target collection not found!"); cleanupAfterDrop(); return; }
    ...
    if (linkBlockData && draggedLinkInfo) { // --- Dropped an existing Link Block ---
        console.log(`Moving link block to collection ${targetCollectionId}`);
        const { linkId, sourceCollectionId, linkData } = draggedLinkInfo;
        if (sourceCollectionId === targetCollectionId) { cleanupAfterDrop(); return; }
        if (targetCollection.links.some(link => link.url === linkData.url)) {
            ...
            cleanupAfterDrop(); return;
        }
        const sourceCollection = collections.find(c => c.id == sourceCollectionId);
        const linkIndexInSource = sourceCollection?.links.findIndex(l => l.id == linkId);
        if (!sourceCollection || linkIndexInSource === -1) { console.error("Could not find source to move."); cleanupAfterDrop(); return; }
        ...
        renderCollections(); saveData();

    } else if (newTabData && draggedTab) { // --- Dropped a New Tab ---
    ...
    } else { console.warn("Drop event occurred with unexpected data."); }
    cleanupAfterDrop();
  }

  function cleanupAfterDrop() { // General cleanup after any drop on collection
    if (dragOverCollectionId) {
        collectionsContainer.querySelector(`.collection[data-collection-id="${dragOverCollectionId}"]`)?.classList.remove('drag-over');
    }
    draggedTab = null;
  ...
  function handleCollectionDragOver(event) {
    ...
    if (!targetElement || targetElement.dataset.collectionId === draggedCollectionId) {
         clearReorderIndicators(); reorderTargetInfo = null; event.dataTransfer.dropEffect = 'none'; return;
    }
    ...
    if (hoverPosition && (!reorderTargetInfo || reorderTargetInfo.element !== targetElement || reorderTargetInfo.position !== hoverPosition)) {
        ...
        clearReorderIndicators(); reorderTargetInfo = null;
    }
  ...
  function handleCollectionDrop(event) {
    ...
    if (!draggedCollectionId || !reorderTargetInfo) {
        ...
        draggedCollectionId = null; reorderTargetInfo = null; return;
    }
    ...
    draggedCollectionId = null; reorderTargetInfo = null;
    ...
    if (draggedIndex === -1 || targetIndex === -1 || draggedIndex === targetIndex) { console.error("Drop failed: Invalid indices."); return; }
  ...
  function finishEditingLink(save = true) {
    ...
    if (!linkBlockElement) { resetEditState(); return; }
    const titleDisplay = linkBlockElement.querySelector('.link-title-display');
    const editInput = linkBlockElement.querySelector('.link-title-input');
    if (!titleDisplay || !editInput) { resetEditState(); return; }
    ...
    if (link && link.title !== finalTitle) { link.title = finalTitle; stateChanged = true; }
    titleDisplay.textContent = finalTitle;
    titleDisplay.style.display = 'block';
    editInput.style.display = 'none';
    const wasEditing = editingLinkId;
    resetEditState();
    if (stateChanged && wasEditing) { console.log("Link title updated, saving..."); saveData(); }
  }

  function handleEditBlur(event) {
    setTimeout(() => { if (editingLinkId && editingLinkId.linkId == event.target.dataset.linkId) { finishEditingLink(true); } }, 150);
  }

  function handleEditKeydown(event) {
    if (event.key === 'Enter') { event.preventDefault(); finishEditingLink(true); }
    else if (event.key === 'Escape') { finishEditingLink(false); }
  }
  ...
  function addCollection() {
    ...
    setTimeout(() => {
        ...
        else { console.warn("Could not find input for new collection to focus."); }
    }, 0);
  }
  ...
  function deleteLink(collectionId, linkId) {
     ...
     renderCollections(); saveData(); console.log(`Deleted link ${linkId}`);
  }

   function deleteCollectionById(collectionId) {
     const index = collections.findIndex(c => c.id == collectionId);
     if (index === -1) return;
     if (confirm(`Delete "${collections[index].name}"?`)) { collections.splice(index, 1); renderCollections(); saveData(); console.log(`Deleted collection ${collectionId}`); }
   }
   ...
   function updateCollectionTitle(collectionId, newName) {
     ...
     if (collection && collection.name !== trimmedName) {
         ...
         saveData(); console.log(`Updated title for collection ${collectionId}`);
  ...
  function navigateToLink(event, url) {
    ...
    if (isExtensionContext() && browser.tabs) {
        ...
        else browser.tabs.getCurrent().then(t => { if (!t || !t.id) throw new Error("No tab ID"); return browser.tabs.update(t.id, { url: url }); }).catch(e => window.location.href = url);
    } else { if (openInNewTab) window.open(url, '_blank'); else window.location.href = url; }
  }
  ...
  function toggleCollectionCollapse(collectionId) {
    ...
    updateCollapseAllButtonState(); saveData();
  }
  ...
  async function loadData() {
    ...
    if (!isExtensionContext() || !browser.storage) { console.warn("Storage API not available."); collections = []; renderCollections(); return; }
    try {
    ...
    } catch (error) { console.error("Error loading data:", error); collections = []; }
    finally { renderCollections(); updateCollapseAllButtonState(); }
  }

  async function saveData() {
    if (!isExtensionContext() || !browser.storage) { console.warn("Storage API not available. Save skipped."); return; }
  ...
  async function fetchOpenTabs() {
    ...
    if (!isExtensionContext() || !browser.tabs) { console.warn("Tabs API not available."); openTabs = []; renderTabsList(); return; }
    try {
    ...
    } catch (error) { console.error("Error fetching tabs:", error); openTabs = []; }
    finally { renderTabsList(); }
  }
  ...
  function importData(event) {
    ...
    if (file.size > maxSize) { alert(`File too large (Max ${maxSize / 1024 / 1024}MB).`); if(inputElement) inputElement.value = null; return; }
    ...
    reader.onload = (e) => {
      ...
      try {
        ...
        if (replace) {
             if (confirm("Replace ALL current collections?")) { collections = importedCollections; alert("Import successful! Collections replaced."); }
             else { console.log("Import (replace) cancelled."); return; }
        } else { collections = collections.concat(importedCollections); alert(`Import successful! ${importedCollections.length} collections added.`); }

        renderCollections(); saveData(); updateCollapseAllButtonState();
      } catch (error) { console.error('Import error:', error); alert(`Import failed: ${error.message}`); }
      finally { if (inputElement) inputElement.value = null; }
    };
    reader.onerror = (e) => { console.error("File read error:", e); alert("Failed to read file."); if (inputElement) inputElement.value = null; };
    ...
    try {
        ...
        const url = URL.createObjectURL(blob);
         if (!isExtensionContext() || !browser.downloads) {
             ...
             const a = document.createElement('a'); a.href = url; a.download = 'my-tab-blocks-config.json';
             document.body.appendChild(a); a.click(); document.body.removeChild(a);
             URL.revokeObjectURL(url); return;
        }
        browser.downloads.download({ url: url, filename: 'my-tab-blocks-config.json', saveAs: true })
            .then(id => URL.revokeObjectURL(url))
            .catch(err => { console.error("Download failed:", err); URL.revokeObjectURL(url); });
    } catch (error) { console.error("Export error:", error); alert("Failed to export."); }
  }
  ...
  if (isExtensionContext()) {
       browser.storage.onChanged.addListener((changes, area) => {
          if (area === 'local' && changes.collections) { console.log("Storage changed externally, reloading..."); loadData(); }
       });
       // Optional: Add tab listeners for auto-refresh
  }
...
//   let draggedTab = null; // Info about the tab being dragged {title, url, favIconUrl?}
//   let draggedCollectionId = null; // ID of the collection being dragged for reordering
//   let dragOverCollectionId = null; // ID of collection being dragged over (for links)
//   let reorderTargetInfo = null; // { element: targetElement, position: 'top' | 'bottom', targetId: string }
//   let editingLinkId = null; // ID of the link currently being edited {collectionId, linkId}
//   let editingLinkOriginalTitle = '';
//   let allCollapsed = false; // State for the "Collapse All" button
...
//   const expandAllIconSVG = `<svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><polyline points="7 13 12 18 17 13"></polyline><polyline points="7 6 12 11 17 6"></polyline></svg>`;
//   const dragHandleIconSVG = `<div class="f-icon f-fill"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13 4C13 4.55228 12.5523 5 12 5C11.4477 5 11 4.55228 11 4C11 3.44772 11.4477 3 12 3C12.5523 3 13 3.44772 13 4Z" fill="currentColor"/><path d="M13 9C13 9.55228 12.5523 10 12 10C11.4477 10 11 9.55228 11 9C11 8.44772 11.4477 8 12 8C12.5523 8 13 8.44772 13 9Z" fill="currentColor"/><path d="M13 14C13 14.5523 12.5523 15 12 15C11.4477 15 11 14.5523 11 14C11 13.4477 11.4477 13 12 13C12.5523 13 13 13.4477 13 14Z" fill="currentColor"/><path d="M13 19C13 19.5523 12.5523 20 12 20C11.4477 20 11 19.5523 11 19C11 18.4477 11.4477 18 12 18C12.5523 18 13 18.4477 13 19Z" fill="currentColor"/><path d="M8 4C8 4.55228 7.55228 5 7 5C6.44772 5 6 4.55228 6 4C6 3.44772 6.44772 3 7 3C7.55228 3 8 3.44772 8 4Z" fill="currentColor"/><path d="M8 9C8 9.55228 7.55228 10 7 10C6.44772 10 6 9.55228 6 9C6 8.44772 6.44772 8 7 8C7.55228 8 8 8.44772 8 9Z" fill="currentColor"/><path d="M8 14C8 14.5523 7.55228 15 7 15C6.44772 15 6 14.5523 6 14C6 13.4477 6.44772 13 7 13C7.55228 13 8 13.4477 8 14Z" fill="currentColor"/><path d="M8 19C8 19.5523 7.55228 20 7 20C6.44772 20 6 19.5523 6 19C6 18.4477 6.44772 18 7 18C7.55228 18 8 18.4477 8 19Z" fill="currentColor"/><path d="M18 4C18 4.55228 17.5523 5 17 5C16.4477 5 16 4.55228 16 4C16 3.44772 16.4477 3 17 3C17.5523 3 18 3.44772 18 4Z" fill="currentColor"/><path d="M18 9C18 9.55228 17.5523 10 17 10C16.4477 10 16 9.55228 16 9C16 8.44772 16.4477 8 17 8C17.5523 8 18 8.44772 18 9Z" fill="currentColor"/><path d="M18 14C18 14.5523 17.5523 15 17 15C16.4477 15 16 14.5523 16 14C16 13.4477 16.4477 13 17 13C17.5523 13 18 13.4477 18 14Z" fill="currentColor"/><path d="M18 19C18 19.5523 17.5523 20 17 20C16.4477 20 16 19.5523 16 19C16 18.4477 16.4477 18 17 18C17.5523 18 18 18.4477 18 19Z" fill="currentColor"/></svg></div>`;
//   const deleteIconSVG = `<svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1" style="pointer-events: auto;"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18" style="pointer-events: auto;"></line></svg>`;
//   const editIconSVG = `<i class="fas fa-pencil-alt"></i>`;
...
//   /** Renders a single link block element according to the specified layout */
//   function createLinkElement(link, collectionId) {
//     const linkBlock = document.createElement('div');
//     linkBlock.className = 'link-block';
//     linkBlock.dataset.collectionId = collectionId;
//     linkBlock.dataset.linkId = link.id;
//     linkBlock.title = `Go to: ${link.url}`; // Tooltip for the whole block
...
//     const linkFavicon = document.createElement('img');
//     linkFavicon.className = 'link-favicon';
//     linkFavicon.src = link.favIconUrl || './icons/placeholder-favicon.png'; // Use stored URL or fallback
//     linkFavicon.alt = ''; // Decorative
//     linkFavicon.onerror = () => {
//       linkFavicon.style.display = 'none';
//     }; // Hide on error

//     // Title Area (contains display span and hidden input)
//     const titleArea = document.createElement('div');
//     titleArea.className = 'link-title-area';
...
//     titleDisplay.className = 'link-title-display';
//     titleDisplay.textContent = link.title || 'Untitled Link';
//     titleDisplay.title = link.title || 'Untitled Link'; // Add tooltip to title itself
...
//     editInput.value = link.title || '';
//     editInput.placeholder = 'Link Title';
//     editInput.style.display = 'none'; // Input hidden by default
//     editInput.dataset.linkId = link.id; // For identification during editing
...
//     deleteBtn.title = 'Remove Link';
//     deleteBtn.innerHTML = '&times;';
//     deleteBtn.dataset.action = 'delete-link'; // For event delegation

//     // Edit Button
//     const editBtn = document.createElement('button');
//     editBtn.className = 'edit-link-btn';
//     editBtn.title = 'Edit title';
//     editBtn.innerHTML = editIconSVG; // Use SVG variable
//     editBtn.dataset.action = 'edit-link'; // For event delegation

//     // Append items to Top Row
//     topRow.appendChild(editBtn);
//     topRow.appendChild(linkFavicon);
//     topRow.appendChild(titleArea);
//     topRow.appendChild(deleteBtn);
...
//     const urlDisplay = document.createElement('span');
//     urlDisplay.className = 'link-url';
//     urlDisplay.textContent = link.url; // Display the full URL
//     urlDisplay.title = link.url; // Tooltip shows full URL

//     // Append items to Bottom Row
//     bottomRow.appendChild(urlDisplay);
...
//       const emptyMsg = document.createElement('div');
//       emptyMsg.className = 'empty-collection-message';
//       emptyMsg.textContent = searchTerm ? 'No matching links found in this collection.' : 'Drag tabs from the right sidebar here to add links.';
//       linksGrid.appendChild(emptyMsg);
//     }
...
//     collectionDiv.addEventListener('drop', handleLinkDrop);
//     return collectionDiv;
//   }
...
//     event.preventDefault();
//     const collectionDiv = event.target.closest('.collection');
//     if (!collectionDiv || !draggedTab || draggedCollectionId) {
//       cleanupLinkDragState();
//       return;
//     }
//     const collectionId = collectionDiv.dataset.collectionId;
//     console.log(`Dropping LINK onto collection ID: ${collectionId}`);
//     try {
...
//         return;
//       }
//       const isDuplicate = targetCollection.links.some((link) => link.url === draggedTab.url);
//       if (isDuplicate) {
...
//       const newLink = { id: Date.now() + Math.random(), url: draggedTab.url, title: draggedTab.title || draggedTab.url, favIconUrl: draggedTab.favIconUrl };
//       targetCollection.links.push(newLink);
//       renderCollections(); // Could optimize
//       saveData();
//     } catch (error) {
